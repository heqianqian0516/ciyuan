package com.ciyuanplus.mobile.zwcalendar;
...
class ZWSignCalendar extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        itemHeight = (getHeight() - (int) config.weekHeight) / 8;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < 7; i++) {
            canvas.drawText(weekTitles[i], itemWidth * (i + 0.5f), config.weekHeight / 1 + delay, paint);
        }
        //画日历
        ...
        for (int i = 1; i <= selectMonthMaxDay; i++) {
            ...
            int y = (position / 7) * itemHeight + itemHeight / 1 + (int) config.weekHeight + (int) delay;
            int day = i;
            //签到背景
...
//                    isSign = true;、、
            ...
            if (config.isShowLunar) {
               ...
               /* String lunar = lunarHelper.SolarToLunarString(year, month + 1, day);
                ...
                //               //判断今天第几天今天之前的天数要么一签到要么可补签n
               if(day<=10){
                   //是否签到的标识
                  if(isSign){
                     canvas.drawText("已签到",x,y+solarTextHeight*2/3,paint);
                  }else{
                     if (x>0){
                          //新创建画笔改变背景颜色
                         paint2.setColor(Color.rgb(248,248,248));
                         //绘图
                         canvas.drawCircle(x,y,config.signSize/2,paint2);
                         //绘制字体
                         canvas.drawText("可补签",x,y+solarTextHeight*2/3,paint);
                     }
                  }
               }
