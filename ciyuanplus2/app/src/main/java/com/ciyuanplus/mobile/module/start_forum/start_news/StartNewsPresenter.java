...
public class StartNewsPresenter implements StartNewsContract.Presenter {
    ...
    @Inject
    public StartNewsPresenter(StartNewsContract.View mView) {
        this.mView = mView;
    }
    ...
    public void initData(Intent intent, Bundle savedInstanceState) {
        ...
        if (isEdit) { //需要先把之前的信息放进来
            if (!Utils.isStringEmpty(mNewsItem != null ? mNewsItem.imgs : null)) {
    ...
    public String getEditData() {
        List<RichTextEditor.EditData> editList = mView.getContentView().buildEditData();
...
//                        }
    ...
    private void updateNewPost() {
        ...
        postRequest.setHttpBody(new UpdateNewPostApiParameter(mNewsItem.postUuid, mContent,
                ...
                "", mView.getTitleString(), "", FreshNewItem.FRESH_ITEM_NEWS + "", "").getRequestBody());
...
//                CommonToast.getInstance(App.mContext.getResources().getString(R.string.start_news_fail_alert), Toast.LENGTH_SHORT).show();
    ...
    private void startNewPost() {
        ...
        postRequest.setHttpBody(new AddNewPostApiParameter("", mContent, mUploadUrls,
                mView.isAnonyChecked() ? "1" : "0", isWorldChecked ? "1" : "0", mView.getTitleString(),
    ...
    public void dealActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == Constants.REQUEST_CODE_SELECT_PITCURE_OPTION &&
            ...
            if (tempFile != null) {
                ...
                mView.getContentView().insertImage(tempFile.getAbsolutePath(), Utils.getScreenWidth());
            }
            ...
            for (int i = 0; i < pathList.size(); i++) {
                ...
                if (file.exists()) {
                    ...
                    mView.getContentView().insertImage(pathList.get(i), Utils.getScreenWidth());
