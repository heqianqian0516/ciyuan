...
public class LoginBindPhonePresenter implements LoginBindPhoneContract.Presenter {
    ...
    @Inject
    public LoginBindPhonePresenter(LoginBindPhoneContract.View mView) {
        this.mView = mView;
    }
    ...
    private void timeTask() {
        if (this.timer != null)
            this.timer.cancel();
        this.mTimerCount = LoginBindPhonePresenter.MAX_TIMER_COUNT_DOWN;
...
//            }
    ...
    public void changePhone(String phone, String mCode) {
        StringRequest postRequest = new StringRequest(ApiContant.URL_HEAD + "/api/user/bindingMobile");
        postRequest.setMethod(HttpMethods.Post);
        postRequest.setHttpBody(new BindPhoneApiParameter(phone, mCode, SendSmsCodeApiParameter.SEND_SMS_TYPE_REGISTER).getRequestBody());
